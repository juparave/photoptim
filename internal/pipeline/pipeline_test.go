...
func genJPEG() []byte {
    ...
    for y:=0;y<8;y++ { for x:=0;x<8;x++ { img.Set(x,y,color.RGBA{uint8(x*32),uint8(y*32),0,255}) } }
    var buf []byte
    w := &sliceWriter{&buf}
...
func TestOrchestratorRun(t *testing.T) {
    ...
    var dl, optc, up int
    for ev := range prog {
        if ev.Err != nil { t.Fatalf("unexpected error: %v", ev.Err) }
        switch ev.Phase { case PhaseDownload: dl++; case PhaseOptimize: optc++; case PhaseUpload: up++ }
    }
    select {
    case e := <-errs:
        if e != nil { t.Fatalf("errs: %v", e) }
    default:
    }
    if dl != 2 || optc != 2 || up != 2 { t.Fatalf("phase counts mismatch dl=%d opt=%d up=%d", dl, optc, up) }
}
